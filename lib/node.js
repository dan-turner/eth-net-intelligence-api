...
var TENDERMINT_RPC_PORT = process.env.TENDERMINT_RPC_PORT || '46657';
var TENDERMINT_RPC = 'http://' + RPC_HOST + ':' + TENDERMINT_RPC_PORT;
var ETH_RPC = 'http://' + RPC_HOST + ':' + (process.env.RPC_PORT || '8545');
...
{
	var ethUrl = ETH_RPC;
	console.info('Starting web3 connection to: ' + ethUrl);
...
Node.prototype.getStats = function(forced)
{
	...
	if (this._web3 && (lastFetchAgo >= UPDATE_INTERVAL || forced === true))
	{
		...
		async.parallel({
			...
			{
				var netInfoUrl = TENDERMINT_RPC + '/net_info';
				request(netInfoUrl, { json: true })
					.then(function (info) {
						callback(null, info.result.peers.length);
					})
...
Node.prototype.getPending = function()
{
	...
	if (this._web3)
	{
		...
		var ethResult = request.post(ETH_RPC, { json: true, body: {
			id: Math.floor(Math.random() * 100000),
			method: "txpool_status"
		}});
		var tendermintResult = request.post(TENDERMINT_RPC + '/unconfirmed_txs', { json: true });

		Promise.all([ ethResult, tendermintResult ])
			.then(function (data) {
				var ethInfo = data[0];
				var tmInfo = data[1];
				...
				var ethPending = web3.toDecimal(ethInfo.result.pending);
				var ethQueued = web3.toDecimal(ethInfo.result.queued);

				var pending = [ethPending, ethQueued, tmInfo.result.n_txs].join(' | ');
