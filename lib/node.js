...
Node.prototype.getPending = function()
{
	...
	if (this._web3)
	{
		...
		request.post(ETH_RPC, { json: true, body: params })
			.then(function (info) {
				var pending = web3.toDecimal(info.result.pending) + ' | ' + web3.toDecimal(info.result.queued);

				var unconfirmedUrl = TENDERMINT_RPC + '/unconfirmed_txs';				
				request.post(unconfirmedUrl, { json: true })
					.then(function (info) {
						pending = pending + ' | ' + info.result.n_txs;
					...
					.catch(function (err) {
						console.error('xx>', 'getPending from Tendermint error: ', err);
					});
			})
			.catch(function (err) {
